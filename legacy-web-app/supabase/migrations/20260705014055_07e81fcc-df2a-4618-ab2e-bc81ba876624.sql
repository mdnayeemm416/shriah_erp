...
-- Backfill: any previously approved session is considered already applied
-- EXCEPT session #10 (recovery target). We can't know the id, so leave
-- stock_applied = false for all approved sessions and let the user click
-- "Apply This Stock Count" wherever it's needed.

CREATE OR REPLACE FUNCTION public.reapply_stock_count(_session_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT status, shop_id, stock_applied
    INTO v_status, v_shop, v_applied
  FROM public.stock_count_sessions
  WHERE id = _session_id AND COALESCE(is_deleted,false) = false
  FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Session not found'; END IF;
  IF v_status <> 'approved' THEN RAISE EXCEPTION 'Only approved sessions can be re-applied'; END IF;
  IF COALESCE(v_applied,false) THEN RAISE EXCEPTION 'Stock already applied for this session'; END IF;
  IF NOT public.user_can_access_shop(v_uid, v_shop) THEN RAISE EXCEPTION 'Not authorized'; END IF;

  FOR r IN
    SELECT i.product_id, i.physical_qty
  ...
  UPDATE public.stock_count_sessions
    SET stock_applied = true,
        ...
        stock_applied_by = v_uid,
        updated_at = now()
  WHERE id = _session_id;

  RETURN jsonb_build_object('applied', v_rows);
END $function$;

-- Also flag freshly-approved sessions as applied so future approvals aren't
-- offered the recovery button (approve_stock_count already updates shop_products).
