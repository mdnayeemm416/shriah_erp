...
-- Backfill: temporarily bypass the immutability trigger.
...
UPDATE public.sales_returns sr
   SET return_number = 'SR-' || n.d || '-' || lpad(n.seq::text, 4, '0')
  FROM numbered n
 WHERE sr.id = n.id;

ALTER TABLE public.sales_returns ENABLE TRIGGER sr_no_update;

CREATE UNIQUE INDEX IF NOT EXISTS sales_returns_return_number_uidx
  ON public.sales_returns(return_number);

CREATE OR REPLACE FUNCTION public.process_sales_return(
  _sale_id uuid,
  _items jsonb,
  _refund_type text DEFAULT 'due_reduction'::text,
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(hashtext('sales_returns_seq:' || v_today));
  SELECT COALESCE(MAX((regexp_replace(return_number, '^SR-\d{8}-', ''))::int), 0) + 1
    INTO v_seq
    FROM public.sales_returns
   WHERE return_number LIKE 'SR-' || v_today || '-%';
  v_return_number := 'SR-' || v_today || '-' || lpad(v_seq::text, 4, '0');

  SELECT COALESCE(full_name, email) INTO v_processed_name
  ...
  INSERT INTO public.sales_returns
    (sale_id, invoice_number, customer_id, customer_name, customer_mobile,
     ...
     return_number, reason, processed_by_name)
  VALUES
    (_sale_id, v_sale.invoice_number, v_sale.customer_id, v_sale.customer_name,
     ...
     v_return_number, _reason, v_processed_name)
  ...
  IF _reason IS NULL OR length(trim(_reason)) = 0 THEN
    SELECT reason INTO v_dominant_reason
      FROM public.sales_return_items
     WHERE return_id = v_return_id AND reason IS NOT NULL AND length(trim(reason)) > 0
     GROUP BY reason
     ORDER BY SUM(qty) DESC
     LIMIT 1;
  ELSE
    v_dominant_reason := _reason;
  END IF;
  ...
  SET total_qty = v_total_qty,
      ...
      reason = COALESCE(reason, v_dominant_reason)
